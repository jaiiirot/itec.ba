...
import { Timestamp } from 'firebase/firestore'; // Lo mantenemos por compatibilidad de tipos con tu UI
...
export const resourcesService = {
  ...
  getApprovedResources: async (): Promise<ResourceData[]> => {
    ...
    // Mapeamos el _id de MongoDB al id que usa tu React
  ...
  submitNewResource: async (resourceData: Omit<ResourceData, 'id'>, isDirectPublish: boolean): Promise<string> => {
    // isDirectPublish ahora lo decide el backend leyendo el token (si es admin lo aprueba directo)
    const token = await getToken();
  ...
  rejectPendingResource: async (resourceId: string): Promise<void> => {
    const token = await getToken();
    await fetch(`${API_URL}/${resourceId}`, {
      method: 'DELETE',
      headers: { 'Authorization': `Bearer ${token}` }
    });
  }
