...
export const ChatInterface: React.FC = () => {
  const { user } = useAuth();
  const userEmail = user?.email || 'invitado'; // Identificador para la caché
    ...
    <div className="bg-itec-surface border border-itec-gray rounded-3xl shadow-2xl flex flex-col h-[650px] relative overflow-hidden animate-in fade-in duration-500">
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 custom-scrollbar flex flex-col gap-6 bg-gradient-to-b from-itec-bg/30 to-itec-bg/10">
        {messages.map((msg, index) => (
          <ChatMessage key={index} role={msg.role} text={msg.text} timestamp={msg.timestamp} isAiGenerated={msg.isAiGenerated} />
        ))}
        ...
        {isTyping && (
          <div className="flex w-full justify-start animate-in fade-in duration-200">
            <div className="flex items-end gap-2">
              <div className="w-8 h-8 rounded-full flex items-center justify-center shrink-0 mb-1 overflow-hidden p-1 shadow-sm">
                 <img src="/logo.png" alt="ITEC" className="w-full h-full object-contain" onError={(e) => { (e.target as HTMLImageElement).src = '/logo.png'; }} />
              </div>
